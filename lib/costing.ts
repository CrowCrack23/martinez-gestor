...
/** Crea un lote de entrada de stock con su costo unitario. */
...
export async function movementCost(movementId: string): Promise<number> {
  ...
  if (error) throw error;
  const total = (data ?? []).reduce((s, r) => s + Number(r.quantity) * Number(r.unit_cost), 0);
...
export async function averageCost(product_id: string, warehouse_id: string): Promise<number> {
  ...
  return qty > 0 ? Math.round((value / qty) * 100) / 100 : 0;
}
...
export const stockValuation = unstable_cache(
  async (): Promise<Record<string, { qty: number; avg_cost: number; value: number }>> => {
    const sb = getSupabase();
    const { data, error } = await sb
      .from("inventory_lots")
      .select("product_id, warehouse_id, unit_cost, qty_remaining")
      .gt("qty_remaining", 0);
    if (error) throw error;
    const acc: Record<string, { qty: number; value: number }> = {};
    for (const l of data ?? []) {
      const key = `${l.product_id}::${l.warehouse_id}`;
      const cur = acc[key] ?? { qty: 0, value: 0 };
      cur.qty += Number(l.qty_remaining);
      cur.value += Number(l.qty_remaining) * Number(l.unit_cost);
      acc[key] = cur;
    }
    const out: Record<string, { qty: number; avg_cost: number; value: number }> = {};
...
export async function setOpeningLotCost(lotId: string, unitCost: number): Promise<void> {
  ...
  const { error: uErr } = await sb
    .from("inventory_lots")
    .update({ unit_cost: Math.round(unitCost * 100) / 100 })
    .eq("id", lotId);
  if (uErr) throw uErr;
  bustCosting();
}
